...
void CClassView::ShowSceneTree(CGScene* scene) //显示图形场景内容
{
	//场景树展示
	m_wndClassView.DeleteAllItems();
	if (scene != nullptr)
	{
		HTREEITEM hRoot = m_wndClassView.InsertItem(_T("图形场景"), 0, 0);
		m_wndClassView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);
		HTREEITEM hClass = m_wndClassView.InsertItem(_T("图形对象"), 1, 1, hRoot);
		CGRenderable* pr = nullptr;
		unsigned i = 0;
		while (pr = scene->getRenderable(i))
		{
	...
	if (hItem != NULL) {
		...
		if (parent != nullptr)
		{
			...
			if (strText == _T("图形对象"))
			{
				...
				if (mScene != nullptr) {
					//实现单选（每次只选一个），先清空选择集
					bool ret = mScene->RenderableUnselectAll();
					//有清除标记或被选中，更新显示
					if (mScene->RenderableSelected(strText) != nullptr || ret == true)
					{
...
void CClassView::UnselectSceneTree() //撤销所有选择
{
	m_wndClassView.SelectItem(nullptr); //树控件撤销选取
	if (mScene != nullptr) {
		mScene->RenderableUnselectAll(); //调用场景类的撤销选取
		CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
		if (pFrame == nullptr)
			return;
		CDocument* pDoc = pFrame->MDIGetActive()->GetActiveView()->GetDocument();
		if (pDoc != nullptr)
			pDoc->UpdateAllViews(nullptr); //更新客户区中图形的绘制
	}
}