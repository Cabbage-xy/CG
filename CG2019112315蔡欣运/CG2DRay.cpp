...
CG2DRay::CG2DRay(const Vec2d& s, const Vec2d& e)
	: CG2DRenderable(CString("CG2DRay")), mStart(s)
{
	Vec2d dir = e - s;
...
/*
CG2DRay::CG2DRay(const Vec2d& s, const Vec2d& dir)
	: CG2DRenderable(CString("直线")), mStart(s)
{
	double dirlen = dir.length();
	double length = 5;
	if (dirlen)
...
*/
CG2DRay::CG2DRay(const CG2DRay& other)
	: CG2DRenderable(CString("CG2DRay")), mStart(other.mStart), mEnd(other.mEnd)
{
	mName.Format(_T("射线-%ld"), sID);
}
CG2DRay::~CG2DRay()
...
void CG2DRay::Render(CGRenderContext* pRC, CGCamera* pCamera) //绘制对象，使用指定相机
{
...
void CG2DRay::computeBoundingBox() //计算包围盒
{
	mABox.setNull(); //先清空
...
bool CG2DRay::Picked(const Vec2d& p, double radius) //是否拾取到
{
